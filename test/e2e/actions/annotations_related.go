...
func DeleteDeploymentCRWithKeepAnnotation(testData *model.TestDataProvider) {
	...
	By(fmt.Sprintf("Try to delete CR %s", testData.InitialDeployments[0].GetName()), func() {
		...
		By("Checking Cluster still existed", func() {
			...
			state := aClient.GetDeployment(testData.Project.Status.ID, testData.InitialDeployments[0].Spec.DeploymentSpec.Name).StateName
...
func RedeployDeployment(testData *model.TestDataProvider) {
	By("Redeploying the cluster", func() {
		deployment := data.CreateDeploymentWithKeepPolicy(testData.Resources.Namespace, testData.InitialDeployments[0].GetName())
		...
		Eventually(func() bool {
			...
			return deploymentForCheck.Status.StateName == "IDLE" // TODO: move to const
