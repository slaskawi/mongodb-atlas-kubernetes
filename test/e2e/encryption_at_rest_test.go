...
var _ = Describe("UserLogin", Label("encryption-at-rest"), func() {
	...
	DescribeTable("Namespaced operators working only with its own namespace with different configuration",
		func(test *model.TestDataProvider, encAtRest v1.EncryptionAtRest, roles []cloudaccess.Role) {
			...
			encryptionAtRestFlow(testData, encAtRest, roles)
		},
		Entry("Test[encryption-at-rest-aws]: Can add Encryption at Rest to AWS project", Label("encryption-at-rest-aws"),
			...
			).WithProject(data.DefaultProject("")), // TODO: remove empty namespace after fix
			v1.EncryptionAtRest{
				AwsKms: v1.AwsKms{
...
func encryptionAtRestFlow(userData *model.TestDataProvider, encAtRest v1.EncryptionAtRest, roles []cloudaccess.Role) {
	...
	By("Create KMS", func() {
		...
		Expect(userData.K8SClient.Update(userData.Context, userData.Project)).Should(Succeed()) // TODO: move it to separate function
	})

	By("Check Encryption at Rest status", func() {
		Eventually(func() string {
			return GetEncryptionAtRestStatus(userData)
...
func GetEncryptionAtRestStatus(userData *model.TestDataProvider) string {
	Expect(userData.K8SClient.Get(userData.Context, types.NamespacedName{Name: userData.Project.Name,
		Namespace: userData.Resources.Namespace}, userData.Project)).Should(Succeed())
	for _, condition := range userData.Project.Status.Conditions {
		if condition.Type == status.EncryptionAtRestReadyType {
