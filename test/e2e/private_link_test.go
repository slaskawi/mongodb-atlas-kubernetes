...
func privateFlow(userData *model.TestDataProvider, requstedPE []privateEndpoint) {
	...
	By("Check if project statuses are updating, get project ID", func() {
		Eventually(func() string {
			return GetProjectPEServiceStatus(userData)
	...
	By("Check statuses", func() {
		Eventually(func() string {
			return GetProjectPEEndpointStatus(userData)
		}).WithTimeout(5 * time.Minute).WithPolling(20 * time.Second).Should(Equal("True"))
		Eventually(func() string {
			return GetReadyProjectStatus(userData)
		}).WithTimeout(5 * time.Minute).WithPolling(20 * time.Second).Should(Equal("True"))
...
func GetProjectPEEndpointStatus(testData *model.TestDataProvider) string {
	...
	for _, condition := range testData.Project.Status.Conditions {
		if condition.Type == status.PrivateEndpointReadyType {
...
func GetProjectPEServiceStatus(testData *model.TestDataProvider) string {
	Expect(testData.K8SClient.Get(testData.Context, types.NamespacedName{Name: testData.Project.Name, Namespace: testData.Project.Namespace}, testData.Project)).Should(Succeed())
	for _, condition := range testData.Project.Status.Conditions {
		if condition.Type == status.PrivateEndpointServiceReadyType {
			return string(condition.Status) // TODO: remake check
		}
	}
	return ""
