	...
	By("Add integration", func() {
		...
		By("Create Secret for integration", func() {
			for _, i := range data.Project.Spec.Integrations {
				Expect(kubecli.CreateUserSecret(data.Context, data.K8SClient, key, i.APIKeyRef.Name, i.APIKeyRef.Namespace)).Should(Succeed())
			}
		})
		...
		Eventually(func() string {
			return GetReadyProjectStatus(data)
		}).WithTimeout(5 * time.Minute).WithPolling(20 * time.Second).Should(Equal("True"))
	})
	atlasClient := atlas.GetClientOrFail()
	By("Check statuses", func() {
		var projectStatus string
		projectStatus, err := kubecli.GetProjectStatusCondition(data.Context, data.K8SClient, status.IntegrationReadyType, data.Resources.Namespace, data.Project.GetName())
